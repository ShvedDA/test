...
class FirmwareFrameDevelop(StandardFrame):
    ...
    def __init__(self, master, **kwargs):
        ...
        # делим фрейм условно на 2 части
        for i in range(2):
        ...
        self.pstTableFrame = TableWithHeaderFrame(
            self,
            header_text="Прошивка PST-контроллера (стадия \"Разработка\")",
            columns=self.table_columns
        )
        self.pstTableFrame.grid(row=0, column=0, pady=(5, 0), sticky="nsew")

        # таблица с прошивками позиционера
        self.posTableFrame = TableWithHeaderFrame(
            self,
            header_text="Прошивка позиционера (стадия \"Разработка\")",
            columns=self.table_columns
        )
        self.posTableFrame.grid(row=1, column=0, sticky="nsew")
        ...
        self.command_buttons.grid(row=2, column=0, pady=(0, 5), sticky='w')
        ...
        self.pstTableFrame.bootTableFrame.treeview.bind("<<TreeviewSelect>>",
                                                         self.pstTableFrame.bootTableFrame.on_select)
        ...
        if hasattr(self.master, "async_update_table"):
            for table in self.dict_firmwares:
                self.master.async_update_table(
                    data_func=lambda tbl=table: self.api_manager.get_all_firmware_files(tbl, True),
    ...
    def add_new_firmware(self):
        ...
        self.current_window = AddNewFirmwareWindow(self, self.get_all_data(), True)
        self.check_queue()
            ...
            try:
                for firmware in firmwares:
                    ...
                    if str_to_bool(firmware.get("is_active")):
                        self.api_manager.update_firmware_status(clicked_table, firmware_id, is_active=False, dev_mode=True)

                success = self.api_manager.update_firmware_status(clicked_table, firmware_data[0], is_active=True, dev_mode=True)
